...
const RELEASE_ASSET_NAME: &str = "stealthy-usage-monitor.exe";
...
const WINGET_PACKAGE_ID: &str = "Xilous.StealthyUsageMonitor";
...
fn updates_dir() -> Result<PathBuf, String> {
    dirs::data_local_dir()
        .map(|dir| dir.join("StealthyUsageMonitor").join("updates"))
        .or_else(|| {
            Some(
                std::env::temp_dir()
